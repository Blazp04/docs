...
## 🔒 Type Safety & Security Standards

### Type-Safe Queries with Drizzle

<CodeGroup>
```typescript Good Examples
// ✅ GOOD: Type-safe queries with compile-time validation
async findByEmail(email: string): Promise<User | null> {
  const [result] = await db
    .select()
    .from(users)
...
// ✅ GOOD: Type-safe partial updates
...
// ❌ BAD: Using raw SQL loses type safety
...
## 🗄️ Drizzle Kit Migration System
...
export class UserService {
  ...
  // ✅ GOOD: Efficient joins with selected fields
...
<AccordionGroup>
  ...
  <Accordion title="Handle Retries Gracefully">
    ```typescript
    // ✅ GOOD: Handle transaction errors gracefully
  ...
  <Accordion title="Avoid Long-Running Transactions">
    ```typescript
    // ❌ BAD: Long-running transactions can cause deadlocks
