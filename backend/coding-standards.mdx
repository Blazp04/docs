...
// ✅ GOOD: Use type unions for simple alternatives
...
```typescript
// ✅ GOOD: Well-structured function with clear flow
export async function createUser(userData: CreateUserInput): Promise<ApiResponse<User>> {
  try {
    // 1. Validation
...
```typescript
// ✅ GOOD: Organized class with clear sections
export class AuthService {
...
```typescript
// ✅ GOOD: Use descriptive error codes in UPPER_SNAKE_CASE
...
```typescript
// ✅ GOOD: Consistent error response pattern with error codes
export async function createUser(userData: CreateUserInput): Promise<ApiResponse<User>> {
  try {
...
<Tabs>
  <Tab title="Named Exports">
    ```typescript
    // ✅ GOOD: Named exports for utilities and services
    export class AuthService { }
  ...
  <Tab title="Default Exports">
    ```typescript
    // ✅ GOOD: Default export for route modules only
